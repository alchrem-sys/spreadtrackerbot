...
def get_all_prices(symbol):
    """Сканує ВСІ біржі без синтаксичних помилок"""
...
async def handle_prices(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        context.user_data.update({"entry1": entry1, "entry2": entry2, "amt": amount, "sym": symbol})
...
async def set_interval(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text(f"🚀 {data['sym']} СПРЕД {mins}хв!\n/status /stop")
        return ConversationHandler.END
    except:
...
async def run_monitor(uid, app):
    ...
    while uid in tasks_store:
        try:
            prices = get_all_prices(data["sym"])
            valid_prices = {k: v for k, v in prices.items() if v is not None}
            ...
            if len(valid_prices) >= 2:
                ...
                pnl = data["amt"] * (max_p - min_p)
                
                text = f"🔥 {data['sym']} LIVE\n\n"
                for ex, p in prices.items():
                    status = f"${p:,.0f}" if p else "❌"
                    text += f"{ex:<8}: {status}\n"
